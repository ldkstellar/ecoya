...
import { useCreatureDetailModal } from "./use-CreatureDetailModal";
...
const MyMarker = ({ item, handleMarkerPress }) => {
  return (
    <Marker
      ...
      coordinate={{
        latitude: item.creatureLatitude,
        longitude: item.creatureLongitude,
      }}
    />
  );
};
...
const Markers = () => {
  const { selectedMarkerData, handleMarkerPress, closeModal, isModalVisible } =
